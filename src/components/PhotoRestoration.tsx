    ...
    runRestore(async () => {
      try {
        ...
        if (result) {
          ...
          showToast("Imagen restaurada con éxito.", "success");
        }
      } catch (e: any) {
        showToast(e.message || "Error al restaurar imagen.", "error");
        throw e;
      }
    });
  };
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
        ...
        if (addWatermark) {
            ...
            ctx.font = `500 ${fontSize}px Rajdhani`;
            ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
            ctx.textAlign = 'right';
            ctx.textBaseline = 'bottom';
            ctx.shadowColor = 'rgba(0, 0, 0, 0.8)';
            ctx.shadowBlur = 5;
            ctx.fillText("Editado con OmniPhoto IA Pro", canvas.width - 15, canvas.height - 15);
        }
        ...
        showToast("Imagen restaurada descargada.", "success");
    };
