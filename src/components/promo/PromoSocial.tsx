...
const PromoSocial: React.FC = () => {
  ...
  const handleGenerateVideo = async () => {
    if (!sourceImage) {
        showToast("Por favor, sube una imagen para generar el video.", "error");
        return;
    }
    clearError();
    setVideoUrl(null);
    ...
    try {
        ...
        if (err.message === "API_KEY_INVALID") {
            ...
            showToast(err.message || "Error al generar video.", "error");
        }
    }
  };
  ...
  const handleSuggestTexts = async () => {
      ...
      try {
          ...
          showToast("Variaciones de texto sugeridas.", "success");
      } catch (e: any) {
          fail(e.message || 'Error al sugerir textos.');
          showToast(e.message || "Error al sugerir textos.", "error");
      }
  }
