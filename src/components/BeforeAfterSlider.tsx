...
interface BeforeAfterSliderProps {
  beforeImage: string;
  afterImage: string;
  style?: React.CSSProperties; // New prop for external styles
}

const BeforeAfterSlider: React.FC<BeforeAfterSliderProps> = ({ beforeImage, afterImage, style }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-full overflow-hidden select-none rounded-xl bg-brand-secondary cursor-ew-resize"
      onMouseDown={handleMouseDown}
      onTouchStart={handleMouseDown}
    >
      <img src={beforeImage} alt="Original" className="absolute top-0 left-0 w-full h-full object-contain" style={{ imageRendering: 'crisp-edges', ...style }} draggable="false" />
      <div
        className="absolute top-0 left-0 w-full h-full overflow-hidden"
        style={{ clipPath: `inset(0 ${100 - sliderPosition}% 0 0)` }}
      >
        <img src={afterImage} alt="Editada" className="absolute top-0 left-0 w-full h-full object-contain" style={{ imageRendering: 'crisp-edges', ...style }} draggable="false" />
      </div>
      <div
